...
class Spike(Sprite):
    def __init__(self,pos,surf,groups,radius,speed,start_angle,end_angle,z=Z_LAYERS['main']):
        ...
        x = self.center[1] + self.radius*cos(radians(self.angle))
...
class Node(pygame.sprite.Sprite):
    def __init__(self,pos,surf,groups,level,data,paths):
        ...
        self.data = data
        self.paths = paths
        self.grid_pos = (int(pos[0]/TILE_SIZE),int(pos[0]/TILE_SIZE))

    def can_move(self,direction):
        if direction in list(self.paths.keys()) and int(self.paths[direction][0])<=self.data.unlocked_level:
            return True
...
class Icon(pygame.sprite.Sprite):
    ...
    def point_collision(self):
        ...
        if self.direction.x == 1 and self.rect.centerx>= self.path[0][0] or \
            ...
            self.rect.centery = self.path[0][0]
            del self.path[0]
            self.find_path()
