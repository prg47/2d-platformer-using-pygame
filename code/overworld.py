...
from sprites import Sprite,AnimatedSprites,Node,Icon,PathSprites
    ...
    def setup(self,tmx_map,overworld_frames):
        ...
        for col in range(tmx_map.width):
            for row in range(tmx_map.height):
                AnimatedSprites((col*TILE_SIZE,row*TILE_SIZE),overworld_frames['water'],self.all_sprites,Z_LAYERS['bg'])
        #objects
        for obj in tmx_map.get_layer_by_name('Objects'):
            if obj.name == 'palm':
                AnimatedSprites((obj.x,obj.y),overworld_frames['palms'], self.all_sprites,Z_LAYERS['main'],randint(4,6))
            else:
                if obj.name=='grass':
        ...
        for obj in tmx_map.get_layer_by_name('Paths'):
            ...
            start_path =obj.properties['start']
            end_path = obj.properties['end']
            self.paths[end_path] = {'pos':pos,'start':start_path}
        ...
        for obj in tmx_map.get_layer_by_name('Nodes'):
            ...
            if obj.name == 'Node':
                avail_paths = {k:v for k,v in obj.properties.items() if k in ['left','right','up','down']}
                ...
                if index>0 and index<len(path)-1:
                    ...
                    if prev_tile.x == next_tile.x:
                        ...
                        if prev_tile.x == -1 and next_tile.y == -1 or prev_tile.y==-1 and next_tile.x==-1:
                            ...
                            surf = self.path_frames['horizontal']
                    PathSprites(pos=(tile.x*TILE_SIZE,tile.y*TILE_SIZE),
                    surf=surf,
